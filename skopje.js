...
function resetInactivityTimer() {
    // Clear any existing timer and set a new one for 5 minutes (300,000 milliseconds)
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        // Fetch data from Firebase for Skopje
        const businessesRef = ref(database, 'businesses/');
        const querySkopje = query(businessesRef, orderByChild('town'), equalTo('Skopje'));
        
        onValue(querySkopje, (snapshot) => {
